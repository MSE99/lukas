defmodule LukasWeb.Operator.CourseSettingsLive do
  ...
  def render(assigns) do
    ...
    <.async_result assign={@loading}>
      ...
      <.button phx-click={show_modal("confirm-delete-modal")}>Delete</.button>

      <.modal id="confirm-delete-modal" danger>
        <div class="mb-10 text-lg">
          <%= gettext("Are you sure you want to delete this course ?") %>
        </div>

        <div class="flex justify-end gap-3">
          <button
            phx-click="delete-course"
            class="bg-red-600 hover:bg-red-800 px-5 py-2 rounded-lg text-white font-bold"
          >
            <%= gettext("delete") %>
          </button>

          <.button phx-click={hide_modal("confirm-delete-modal")}>
            <%= gettext("cancel") %>
          </.button>
        </div>
      </.modal>
  ...
  def handle_event("delete-course", _, socket) do
    {:ok, _} = Learning.delete_course(socket.assigns.course)
    {:noreply, push_navigate(socket, to: ~p"/controls/courses")}
  end
