defmodule LukasWeb.Lecturer.CourseLessonsLive do
  ...
  defp load_course_with_lessons(%{"id" => raw_id}, current_user) do
    with {id, _} <- Integer.parse(raw_id),
         ...
         lecturers <- Staff.list_course_lecturers(course),
         lect when lect != nil <- find_lecturer_in(lecturers, current_user) do
  ...
  defp find_lecturer_in(lecturers, lect) do
    Enum.find(lecturers, fn other -> other.id == lect.id end)
  end
  ...
  def render(assigns) do
    ...
    <.async_result assign={@loading}>
      ...
      <.modal :if={@show_form_modal} id="form-modal" on_cancel={JS.push("clear")} show>
        <img
          :if={@lesson}
          src={~p"/tutor/my-courses/#{@course.id}/lessons/#{@lesson.id}/image"}
