defmodule LukasWeb.Lecturer.CourseSettingsLive do
  ...
  def mount(%{"id" => raw_id}, _, socket) do
    case Integer.parse(raw_id) do
      {id, _} ->
        next_socket =
          ...
          |> start_async(:loading, fn ->
            {course, lecturers, course_tags} = Course.Staff.get_course_with_lecturers(id)
            tags = Categories.list_tags()
            %{course: course, course_tags: course_tags, tags: tags, lecturers: lecturers}
          end)
      ...
      _ ->
        {:ok, redirect(socket, to: ~p"/tutor/my-courses")}
    end
  end
  ...
  def handle_async(:loading, {:ok, result}, socket) do
    %{course: course, course_tags: course_tags, tags: tags, lecturers: lecturers} = result

    if socket.assigns.current_user not in lecturers do
      {:noreply, redirect(socket, to: ~p"/tutor/my-courses")}
    else
