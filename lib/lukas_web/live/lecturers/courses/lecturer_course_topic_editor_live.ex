defmodule LukasWeb.Lecturer.TopicEditorLive do
  ...
  def handle_async(:loading, {:exit, _}, socket) do
    {:noreply,
     ...
     |> redirect(to: ~p"/tutor/my-courses")}
  end
    ...
    case res do
      {:ok, _} ->
        {:noreply,
         redirect(socket,
           to:
             ~p"/tutor/my-courses/#{socket.assigns.course.id}/lessons/#{socket.assigns.lesson.id}"
         )}
