defmodule LukasWeb.Lecturers.LessonLiveTest do
  ...
  test "should require an authenticated admin.", %{conn: conn} do
    assert {:error, {:redirect, _}} = live(conn, ~p"/tutor/my-courses/15/lessons/10")
  end
  ...
  def setup_test(ctx) do
    ...
    Map.merge(
      ...
      %{
        ...
        path: ~p"/tutor/my-courses/#{course.id}/lessons/#{lesson.id}",
        new_topic_path: ~p"/tutor/my-courses/#{course.id}/lessons/#{lesson.id}/new-topic"
      }
    )
  end

  describe "index" do
    setup [:register_and_log_in_lecturer, :setup_test]

    test "should redirect if the lesson id is invalid.", %{conn: conn, course: course} do
      assert {:error, {:redirect, _}} =
               live(conn, ~p"/tutor/my-courses/#{course.id}/lessons/invalid")
    end
    ...
    test "should render a button for removing topics.", %{conn: conn, lesson: lesson} do
      {:ok, lv, _html} =
        live(conn, ~p"/tutor/my-courses/#{lesson.course_id}/lessons/#{lesson.id}")
  ...
  describe "edit topic" do
    ...
    test "form should render errors on change.", %{
      ...
      {:ok, lv, _html} =
        live(
          conn,
          ~p"/tutor/my-courses/#{course.id}/lessons/#{lesson.id}/topics/#{topic1.id}/edit-topic"
        )
